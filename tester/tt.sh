...
do
    test=$(seq 1 100 | sort -R) #you can change the size of the numbers list here by modefy the range (1 500) for 100 number use (1 100) or any range that have 100 numbers
    result=$(./push_swap $test | wc -l)
    if [ $result -gt 700 ] #you can change the number of max instractions here by modefy the value (5500) for 500 number use 5500, for 100 use 700 ,5 use 12 ,3 use 2
    then
